...
class Database:
    def __init__(self):
        ...
        self.db = self.client['telegram_bot_db12115']
...
async def trigger_single_bot_engagement(client, peer_str, msg_id, action_type, emoji_list, index=0):
    ...
    try:
        ...
        cache_key = str(peer_str)
        if cache_key in client.entity_cache:
            entity = client.entity_cache[cache_key]
        else:
            try:
                ...
                client.entity_cache[cache_key] = entity
            except ValueError:
                try:
...
async def process_callback_data(event, data):
    ...
    # --- BOT MANAGEMENT ---
        ...
        await event.edit("➕ **𝗔𝗱𝗱 𝗡𝗲𝘄 𝗕𝗼𝘁**\nSelect login method:", buttons=[
            [Button.inline("📱 Phone Number", b"add_phone"), Button.inline("📝 Session String", b"add_string")],
            [Button.inline("📁 Upload .session / .zip", b"add_file")],
        ...
        login_states[chat_id] = {"step": "UPLOAD_FILE"}
        await event.edit("📁 **Upload Session File(s)**\n\nSend a single `.session` file or a `.zip` containing multiple `.session` files.\n*Compatible with Heroku & VPS.*\n\n__Type /cancel to abort.__", 
                         buttons=[[Button.inline("❌ Cancel", b"menu_bots")]])
...
async def wizard_handler(event):
    ...
    # Process cancellations immediately across all steps
    if event.text and event.text.strip() == "/cancel":
        if 'client' in state:
            try: 
                state['client']._vc_stop_flag = True
    ...
    try:
        # --- NEW FILE UPLOAD HANDLER FOR .SESSION AND .ZIP ---
        if step == "UPLOAD_FILE":
            if not event.message.media or not getattr(event.message, 'file', None):
                return await event.respond("❌ Please upload a document (`.session` or `.zip`).")
            
            file_name = event.message.file.name or ""
            if not (file_name.lower().endswith('.session') or file_name.lower().endswith('.zip')):
                return await event.respond("❌ Invalid file type. Only `.session` or `.zip` allowed.")
            
            msg = await event.respond("⏳ **Downloading and processing file...**\n*This may take a moment.*")
            login_states.pop(chat_id, None)
            
            try:
                # Use a secure temporary directory for Heroku/VPS compatibility
                        ...
                        for root, dirs, files in os.walk(temp_dir):
                            for f in files:
                                if f.endswith('.session'):
                                    session_files_to_process.append(os.path.join(root, f))
                    else:
                        session_files_to_process.append(file_path)
                        
                    if not session_files_to_process:
                        return await msg.edit("❌ No `.session` files found in the uploaded archive.")
                    
                    success_count = 0
                    failed_count = 0
                    
                    await msg.edit(f"🔄 **Processing {len(session_files_to_process)} session(s)...**")
                    
                    for s_file in session_files_to_process:
                        # Telethon accepts base path without extension to connect to SQLite
                        base_path = s_file.rsplit('.session', 1)[0]
                        try:
                            # Use temporary client to extract auth key safely without file locks
                            temp_client = TelegramClient(base_path, API_ID, API_HASH)
                            await temp_client.connect()
                            
                            if await temp_client.is_user_authorized():
                                user = await temp_client.get_me()
                                # Convert SQLite data to completely portable String Session
                            ...
                            logger.error(f"Failed to process session file {s_file}: {e}")
                            failed_count += 1
                            
                    await reload_userbots()
                    await msg.edit(f"✅ **Processing Complete!**\n\n🟢 Successfully Added: `{success_count}`\n🔴 Failed / Dead: `{failed_count}`\n\n*(Files instantly cleaned up to save server space)*", buttons=[[Button.inline("🤖 Back to Bots", b"menu_bots")]])
            except Exception as e:
                logger.error(f"File Upload Error: {e}")
                await msg.edit(f"❌ Error processing file: {str(e)[:50]}")
            return


        # Ensure text is present for all remaining text-based wizard steps
